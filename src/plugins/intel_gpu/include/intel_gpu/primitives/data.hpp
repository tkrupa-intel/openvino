        ...
        if (_allocation_type == allocation_type::usm_host || _allocation_type == allocation_type::usm_shared) {
            ...
            if (data_size < DATA_BLOCK_SIZE || output_layout.format.is_image_2d()) {
                ...
                while (dst_offset < data_size) {
                    ...
                    if (buf_flag) {
                        if (is_cache_without_weights) {
                            std::memcpy(reinterpret_cast<uint8_t*>(_buf1.data()), shared_buf->get_ptr<uint8_t>() + dst_offset, copy_size);
                        } else {
                        ...
                        if (is_cache_without_weights) {
                            std::memcpy(reinterpret_cast<uint8_t*>(_buf2.data()), shared_buf->get_ptr<uint8_t>() + dst_offset, copy_size);
                        } else {
